...
class ShiftedEnv(gym.Wrapper):
    """Wrapper that shifts the environment parameters."""

    def __init__(self, env):
        super(ShiftedEnv, self).__init__(env)
        # Change dtype of observation to be float32
        self.observation_space = gym.spaces.Box(
            low=self.observation_space.low,
            high=self.observation_space.high,
...
def get_shifted_half_cheetah_env(param_ids: List[int], param_values: List[float]):
    values = [0.4, 0.1, 0.1, 6.0, 4.5, 6.2502093]
    for i, v in zip(param_ids, param_values):
        values[i] = v
    ...
    env.model.dof_armature[[3, 6]] = values[1:3]
    env.model.dof_damping[[3, 6]] = values[3:5]
    env.model.body_mass[1:2] = values[5:6]

    return env


def get_shifted_reacher_env(param_ids: List[int], param_values: List[float]):
    values = [1.0, 1.0, 1.0, 1.0, 0.0356047, 0.0356047, 0.0041888]
    for i, v in zip(param_ids, param_values):
        values[i] = v
    ...
    env.model.dof_armature[:2] = values[:2]
    env.model.dof_damping[2:4] = values[2:4]
    env.model.body_mass[1:4] = values[4:7]
...
def get_shifted_env(
    ...
    if env_name == "half_cheetah":
        env = get_shifted_half_cheetah_env(ids, values)
    elif env_name == "reacher":
        env = get_shifted_reacher_env(ids, values)
    else:
        raise ValueError(f"Unknown environment: {env_name}")

    env = ShiftedEnv(env)
    env = suite_gym.wrap_env(env, max_episode_steps=max_episode_steps)
    return tf_py_environment.TFPyEnvironment(env)
