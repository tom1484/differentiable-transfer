...
class Critic:
    def __init__(
        ...
        (
            self._net_input_env,
            self._net_input_goal,
            self._net_input_action,
            self._net_input_state,
            self._net_input_history,
        ) = self._net_inputs
        ...
        (
            self._target_input_env,
            self._target_input_goal,
            self._target_input_action,
            self._target_input_state,
            self._target_input_history,
        ) = self._target_inputs

        self._target_network_params = tf.trainable_variables()[
            (len(self._network_params) + num_actor_vars) :
        ]

        # op for initializing the target network with online network weight
        self._initialize_target_network_params = [
            self._target_network_params[i].assign(self._network_params[i])
        ...
        self._update_target_network_params = [
            self._target_network_params[i].assign(
                tf.multiply(self._network_params[i], self._tau)
                + tf.multiply(self._target_network_params[i], 1.0 - self._tau)
            )
            for i in range(len(self._target_network_params))
        ]
        ...
        self._optimize = tf.train.AdamOptimizer(self._learning_rate).apply_gradients(
            zip(self._grad, self._network_params)
        )
        ...
        self._gradients_summaries = [
            tf.summary.scalar("gradient_{}".format(i), grad)
            for i, grad in zip(range(1000), [tf.norm(gr) for gr in self._grad])
        ]
    ...
    def create_network(self):
        ...
        input_history = tflearn.input_data(
            shape=[None, MAX_STEPS, self._dim_action + self._dim_state]
        )
    ...
    def train(
        ...
        net_out, optimize, loss_summary, gradients_summaries = self._sess.run(
            [self._net_out, self._optimize, self._loss_summary, self._merged_gradients],
            feed_dict={
    ...
    def predict_target(
        self, input_env, input_state, input_goal, input_action, input_history
    ):
        return self._sess.run(
            self._target_out,
            feed_dict={
