...
class Actor:
    def __init__(
        ...
        self._net_input_state, self._net_input_goal, self._net_input_history = (
            self._net_inputs
        )
        self._network_params = tf.trainable_variables()

        self._target_inputs, self._target_out, self._target_scaled_out = (
            self.create_network()
        )
        (
        ...
        self._unnormalized_actor_gradients = tf.gradients(
            self._net_scaled_out, self._network_params, -self._action_gradient
        )
        self._actor_gradients = list(
            map(
                lambda x: tf.div(x, self._batch_size),
        ...
        self._num_trainable_vars = len(self._network_params) + len(
            self._target_network_params
        )

    def create_network(self):
        input_state = tflearn.input_data(
            shape=[None, self._dim_state], name="input_state"
        )
